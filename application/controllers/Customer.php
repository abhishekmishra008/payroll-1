...
class Customer extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->db->query("SELECT `user_type`,`firm_logo`,`user_name` FROM `user_header_all` where `firm_id`= '$firm_id'");
        if ($query->num_rows() > 0) {
        ...
        $is_duedate_permitted = $this->db->query("SELECT count(1) as 'due_date_permitted' FROM `partner_header_all` where `due_date_creation_permitted`='1' and `firm_id`= '$firm_id'")->row();
        if ($is_duedate_permitted->due_date_permitted > 0) {
            $data['due_date_permitted'] = 1;
        } else {
            $data['due_date_permitted'] = 0;
        }
        $this->load->view('client_admin/add_customer', $data);
    }

    public function edit_enquiry_cust_fun($customer_id = "", $enquiry_id = "") {
...
//        $email_id = $this->session->userdata('login_session');
        ...
        $record2 = $this->customer_model->get_customer_contact_details1($customer_id, $enquiry_id);
        if ($record2 !== FALSE) {
            $res = $record2->row();
        } else {
            $res = "";
        }
        $query_fetch_cust_details = $this->db->query("SELECT * FROM customer_header_all WHERE customer_id='$customer_id'");
        if ($query_fetch_cust_details->num_rows() > 0) {
            $record = $query_fetch_cust_details->row();
        } else {
            $record = '';
        }
        $data['record'] = $record;
        $data['contact_record_new'] = $res;
//        $data['service_id'] = $service_id;
        $data['enquiry_id'] = $enquiry_id;
        $data['prev_title'] = "Fill Data";
        $data['page_title'] = "Fill Data";
        $this->load->view('employee/edit_customer', $data);
    }

    public function create_enquiry_customer() {
        $customer_id = $this->getEnquiry_CustomerId();
        ...
        if (empty(trim($customer_name))) {
            ...
            $response['id'] = 'customer_contact_number';
            $response['error'] = 'Enter Proper Contact No.';
        } else {

            if ($attached_due_date_id == 'enq') {
            ...
            if ($result !== false) {
                ...
                $qry = $this->db->query("SELECT `reporting_to` FROM `partner_header_all` where `firm_id`= '$firm_id'");
                if ($qry->num_rows() > 0) {
                    $r_data = $qry->row();
                    $boss_id = $r_data->reporting_to;
                }
            }
            ...
            $data = array(
                ...
                'boss_id' => $boss_id,
                'created_on' => $date,
                'attribute_3_value' => $customer_name, //customer name
                'attribute_2_value' => $customer_no, //customer contact number
                'attribute_1_value' => $customer_email, // customer email
            );
            ...
            if ($validte_firm_email == TRUE) {
                ...
                $record = $this->customer_model->add_customer_enq($data, $customer_id, $firm_id);
                if ($record == TRUE) {
// email function code
...
// sms function code
    ...
    public function create_customer() {
        ...
        $country_type = $this->input->post('country_type');
        $firm_id = $this->input->post('ddl_firm_name_fetch');
        $old_customer_id = $this->input->post('customer_id');
        $enquiry_id = $this->input->post('enquiry_id');



        if ($country_type != '0') {
            $customer_country = $country_type;
        }
        ...
        if (isset($firm_id) && empty($firm_id)) {
        ...
        } else if (empty(trim($customer_name))) {
            $response['id'] = 'customer_name';
            $response['error'] = 'Enter Customer Name';
        } elseif (empty($industry_type)) {
            $response['id'] = 'industry_type';
            $response['error'] = 'Enter industry  Type';
        } elseif (empty(trim($customer_type))) {
        ...
        } elseif ($country_type != '0' && empty($state_name)) {
            $response['id'] = 'state_id';
            $response['error'] = 'Enter Customer State';
        } elseif ($country_type != '0' && empty($city_name)) {
            $response['id'] = 'city_id';
            $response['error'] = 'Enter Customer City';
        } elseif ($country_type == '0' && empty(trim($customer_state))) {
            $response['id'] = 'customer_state';
            $response['error'] = 'Enter Customer State';
        } elseif ($country_type == '0' && empty(trim($customer_city))) {
            ...
            $index = '0';
            $folder_info = $this->Nas_modal->folder_creation($index, $firm_id);

            if ($folder_info != false) {
                $response['file_id'] = $folder_info;
            } else {

                $response['file_id'] = '';
            }
            //var_dump($response['file_id']);
            ...
            //  var_dump($firm_id);
            // check firm have duedate creation permission
            $qry = $this->db->query("SELECT `reporting_to`,`due_date_creation_permitted` FROM `partner_header_all` where `firm_id`= '$firm_id'");
            if ($qry->num_rows() > 0) {
                $r_data = $qry->row();
                $boss_id = $r_data->reporting_to;
                $is_duedate_permitted = $r_data->due_date_creation_permitted;
            } else {
                $is_duedate_permitted = 0;
            }

            if ($is_duedate_permitted == '1') {
                if ($attached_due_date_id != null) {
            ...
            if (empty(trim($customer_state)) && empty(trim($customer_city))) {
...
// variable declaration
                ...
                for ($i = 1; $i <= $cust_count; $i++) {
                    ...
                    $view_detail = $this->input->post('view_detail');
...
//var_dump($data_gst);
            ...
            if ($validte_firm_email == TRUE) {
                ...
                if ($record == '1') {
                    ...
                    if (isset($old_customer_id) && isset($enquiry_id)) {
                        $this->db->query("UPDATE `enquiry_header_all` SET `customer_id`='$customer_id' where `customer_id`='$old_customer_id' AND `firm_id`='$firm_id'");
                        $this->db->query("UPDATE `enquiry_transaction_all` SET `customer_id`='$customer_id' where `customer_id`='$old_customer_id' AND `firm_id`='$firm_id'");
...
//generate user_id
    ...
    public function getEnquiry_CustomerId() {
        $result = $this->db->query("SELECT customer_id FROM `enquiry_header_all` where customer_id LIKE 'E%' ORDER BY customer_id DESC LIMIT 0,1");
        if ($result->num_rows() > 0) {
            $data = $result->row();
            $customer_id = $data->customer_id;
//generate user_id
...
// VIEW CUSTOMER
    public function view_customer_data() {
        ...
        $firm_id_fetch = $this->input->post('firm_id_fetch');
        //var_dump($firm_id_fetch);
        if ($user_type == '3') {

            $result = $this->db->query("SELECT * FROM `partner_header_all` WHERE `firm_email_id`='$email_id'");
        } else if ($user_type == '4') {

            $result = $this->db->query("SELECT * FROM `partner_header_all` WHERE `boss_id`=(select linked_with_boss_id from user_header_all where email='$email_id')");
        } else if ($user_type == '2') {

            $data['result'] = '';
            $data['user_type'] = $user_type;
            $this->load->view('client_admin/view_customer', $data);
        ...
        if ($result->num_rows() > 0) {
            ...
            $query = $this->db->query("SELECT * FROM `customer_header_all` WHERE  firm_id='$firm_id' AND activity_status='1'");

            if ($query->num_rows() > 0) {
                $record = $query->result();

                $data = array('task_count' => array(), 'customer_id' => array());
                $total = 0;
//                $completed = 0;
...
// for due date count
                    ...
                    $response['task_due_count'][] = ['customer_id' => $customer_id, 'task_count' => $total, 'duedate_count' => $deu_count];
                }
    ...
    public function view_customer_data_hq() {
        ...
        $selected_firm_id = $this->input->post('firm_id_fetch');
...
//                echo "SELECT * FROM `partner_header_all` WHERE `reporting_to` = '$boss_id' AND firm_id ='$selected_firm_id'";
            ...
            if ($query->num_rows() > 0) {
                ...
                if ($q11->num_rows() > 0) {
                    ...
                    $response['due_date_creation_permitted'] = $due_date_creation_permitted;
                } else {
                    $response['due_date_creation_permitted'] = '';
                }

                $table = array();
                if ($data['cust_data'] != null) {
                    foreach ($data['cust_data'] as $row) {
                        //var_dump($row);
                        $total_assignment = $row['task_count'] + $row['duedate_count'];
                        ...
                        $table[] = array(
                            ...
                            $row['customer_id'],
                            $row['firm_id'],
                            $row['attached_due_date_id'],
                            $row['activity_status']
                        );
                    }

                    $final_table = array(
                        "due_date_creation_permitted" => $response['due_date_creation_permitted'],
                        "data" => $table
                    );

                    //$response['data']=$final_table;
                    //var_dump($response['table']);
                    // $data['selected_firm_name'] = $s_firm_name;
                    $response['message'] = 'success';
                    $response['code'] = 200;
                    $response['status'] = true;
...
//                WHERE c_t_a_a.customer_id='Cust_1001' AND c_t_a_a.sub_task_id = ''
        ...
        $this->db->where("c_t_a_a.customer_id = '$customer_id' AND c_t_a_a.sub_task_id = '' AND  c_t_a_a.status != '5'");
        $query = $this->db->get();


//        foreach ($query->result() as $total_task_assignment) {
//
//            $btn_edit = "<a type='button' class='btn btn-circle green btn-icon-only btn-default'  data-toggle='modal' onclick='show_task_details(\"" . $total_task_assignment->customer_id . "\")'><i class='fa fa-eye'></i></a>";
//            $archive = "<a type='button' class='btn btn-circle grey btn-icon-only btn-default'  data-toggle='modal' onclick='mark_archive(\"" . $total_task_assignment->customer_id . "\")'><i class='fa fa-archive'></i></a>";
//
//
//
//            $status = $total_task_assignment->status;
//            if ($status === '1') {
//                $status = "<span class='label label-sm label-info'> Initiated </span>"; //initiated
//                $edit = $btn_edit;
//            } else if ($status === '2') {
//                $status = "<span class='label label-sm label-warning'> Working </span>"; //working
//                $edit = $btn_edit;
//            } else if ($status === '3') {
//                $status = "<span class='label label-sm label-success'> Completed </span>"; //completed
//                $edit = $btn_edit;
//            } else {
//                $status = "<span class='label label-sm label-danger'> Closed </span>"; //closed
//                $edit = $btn_edit;
//            }
//
//            $data['show_data'] = array(
//                'task_assignment_name'=>$total_task_assignment->task_assignment_name,
//                'task_name'=>$total_task_assignment->task_name,
//                'start_date'=>$total_task_assignment->created_on,
//                'completion_date'=>$total_task_assignment->completion_date,
//                'status'=>$status,
//                'edit'=>$btn_edit,
//                'archive'=>$archive
//            );
...
//        exit();
    ...
    public function get_ddl_customer_enq() {
...
//            $this->db->where("c_t_a_a.status = '$status' and c_t_a_a.customer_id = '$customer_id' and c_t_a_a.task_assignment_id = '$task_assign_id'");
    ...
    //attache due date for customer in client admin
    ...
    public function duedate_Name() {
        ...
        $query2 = $this->db->query("SELECT due_date_name FROM `due_date_header_all` WHERE firm_id='$firm_id'");
    ...
    public function edit_customer_details_view($cust_id) {
        ...
        if ($record !== false && $record2 !== false) {
            ...
            $response['code'] = 200;
            $response['status'] = true;
            $this->load->view('client_admin/edit_customer', $data);
        } else {
            $response['message'] = 'No data to display';
            $response['code'] = 204;
            $response['status'] = false;
    ...
    public function fetch_customer_data() {
        ...
        $total_states = $this->customer_model->state();
        //var_dump($record);
        $customer_detail = $this->customer_model->get_customer_details($cust_id)->result();
        $total_cities = $this->customer_model->city();
        $C_emp_details = $this->customer_model->get_customer_contact_details($cust_id)->result();
        ...
        $firm_name2 = $this->db->query("select * from partner_header_all where firm_id='$firm_id'")->row();

        $Firm_name = $firm_name2->firm_name;
        foreach ($C_emp_details as $row1) {


            $view_detail_permission = $row1->permission;
        }
        foreach ($total_states as $T_states) {

            if (in_array($T_states, $state)) {

            } else {

                $final_state[] = $T_states;
            }
        }

        foreach ($total_cities as $T_cities) {
            if (in_array($T_cities, $city)) {
        ...
        //var_dump($industry_type);
        //var_dump($view_detail_permission);
        //var_dump($final_city);
        if ($customer_detail !== null && $C_emp_details !== null) {

            $response['firm_id'] = $firm_id;
            $response['firm_name'] = $Firm_name;
            $response['customer_type'] = $customer_type;
            $response['country'] = $country;
            $response['industry_type'] = $industry_type;
            $response['state'] = $state;
            $response['All_states'] = $total_states;
            $response['City'] = $city;
            $response['view_detail_permission'] = $view_detail_permission;
    ...
    public function edit_customer() {
        ...
        $country_type = $this->input->post('country_type');
        $customer_type = $this->input->post('customer_type');
        $firm_id = $this->input->post('ddl_firm_name');
...
//var_dump($city_name);
        ...
        if (empty(trim($customer_name))) {
            $response['id'] = 'customer_name';
            $response['error'] = 'Enter Customer Name';
        } elseif (empty(trim($industry_type))) {
            $response['id'] = 'industry_type';
            $response['error'] = 'Enter industry_type Type';
        } elseif (empty(trim($customer_type))) {
            $response['id'] = 'customer_type';
            $response['error'] = 'Enter Customer Type';
        } elseif (empty($customer_no)) {
                ...
                if ($query == true) {
                    for ($i = 1; $i <= $cust_count; $i++) {
                        ...
                        if ((empty((trim($e_name))))) {
                            ...
                            echo json_encode($response);
                            exit();
                        } elseif (empty($e_designation)) {
                            $response['error'] = "Please enter Designation";
                            $response['id'] = 'employee_designation' . $i;
                            echo json_encode($response);
                            exit();
                        } elseif (empty($e_visible)) {
                            $response['error'] = "Please enter Designation";
...
//            var_dump($record);
    ...
    public function GetCustomerDueDate() {
        ...
        $this->db->where("c_d_d_t_t_a.customer_id = '$due_date_cust_id' and c_d_d_t_t_a.firm_id='$firm_id' and c_d_d_t_t_a.status !='5'");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $record = $query->result();
            foreach ($record as $row) {
                ...
                $response['due_date_data'][] = ['due_date_id' => $rs_due_date_id, 'due_date_name' => $row->due_date_name, 'duration' => $row->duration, 'customer_id' => $row->customer_id, 'customer_name' => $row->customer_name, 'due_date_task_count' => $due_date_task_count];
            }
    ...
    public function GetCustomerDueDateTask() {
        ...
        $this->db->where("customer_id = '$due_date_cust_id' and due_date_id = '$due_date_id' and status != '5'");
        ...
        if ($query1->num_rows() > 0) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                foreach ($query->result() as $row) {
                    ...
                    $response['user_data'][] = ['user_name' => $row->user_name, 'user_id' => $row->user_id];
                }
                $response['message'] = 'success';
                $response['code'] = 200;
                $response['status'] = true;
...
//            }
        ...
        if ($check_list == 'SMS' && $check_list == 'email') {
            $auth_key = "178904AVN94GK259e5e87b";
    ...
    public function TransferTaskAssignToArchive() {
        ...
        $this->db->where("customer_id='$customer_id' AND task_assignment_id='$task_assign_id'");
        $query2 = $this->db->update('custom_sub_task_header_all', $data);
        if ($this->db->affected_rows() > 0) {
            $response['message'] = 'success';
            $response['code'] = 200;
            $response['status'] = true;
    ...
    public function get_all_assignment_data() {
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($record as $row) {
                ...
                $this->db->from('customer_due_date_task_transaction_all as c_d_d_t_t_a');
                $this->db->join('due_date_task_header_all as d_d_t_h_a', 'c_d_d_t_t_a.due_date_task_id = d_d_t_h_a.due_date_task_id');
                $this->db->where("c_d_d_t_t_a.customer_id = '$customer_id' and c_d_d_t_t_a.firm_id='$firm_id'");
                ...
                $response['get_due_date_data'][] = ['due_date_id' => $rs_due_date_id, 'due_date_name' => $row->due_date_name, 'firm_id' => $row->firm_id, 'duration' => $row->duration, 'customer_id' => $row->customer_id, 'customer_name' => $row->customer_name, 'due_date_task_count' => $due_date_task_count];
            }
    ...
    public function remove_customer_data() {
        ...
        if ($due_date != NULL && $task_assign != NULL) {
            foreach ($due_date as $value_due) {
                ...
                $this->db->where("customer_id='$customer_id' AND firm_id='$firm_id' AND due_date_id='$value_due'");
