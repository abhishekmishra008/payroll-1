<?php
	...
	class Demp extends CI_Controller {
		...
		public function getParentIdForSelectFirm() {
			
			if (!is_null($this->input->post_get("index"))) {
				 $index = $this->input->post_get("index");
				 $firm_id = $this->input->post_get("firm_id");
				$result = $this->Nas_modal->folder_creation($index,$firm_id);
				if ($result != false) {
					$response["status"] = 200;
					$response["body"] = $result;
					} else {
		...
		public function getParentIdForWorkingPapaer() {
			
			if (!is_null($this->input->post_get("index"))) {
				$index = $this->input->post_get("index");
				$firm_id = $this->input->post_get("firm_id");
				
				$result = $this->Nas_modal->folder_creation($index,$firm_id[0]);
				if ($result != false) {
					$response["status"] = 200;
					$response["body"] = $result;
					} else {
		...
		public function download_nas_file(){
			...
			if($this->db->affected_rows()==1){
				$result=$q->result();
				$response['file_id']=$result[0]->file_id;
		...
		public function get_id_from_user_header_all(){
		$result1 = $this->customer_model->get_firm_id();
			if ($result1 !== false) {
				$user_id = $result1['user_id'];
				$firm_id=$result1['firm_id'];
			...
			$q=$this->db->query("select e_folder_id from user_header_all where user_id='$user_id' and firm_id='$firm_id'")->row();
			//echo $this->db->last_query();
			if($this->db->affected_rows()>0){
				$response['parent_id']=$q->e_folder_id;
				$response['message'] = 'success';
				$response['code'] = 200;
				$response['status'] = true;
			}else{
