...
class Graphs_emp extends CI_Controller {
...
//        $email_id = $this->session->userdata('login_session');
        ...
        $query_to_fetcth_event_date = $this->db->query("SELECT `holiday_date`,`create_at`,`end_date`,`holiday_id` FROM `holiday_header_all` where  FIND_IN_SET(`holiday_applied_in`, '$firm_id')");
        if ($query_to_fetcth_event_date->num_rows() > 0) {
            ...
            foreach ($record_event as $row) {
                ...
                $query_event = $this->db->query("SELECT * FROM holiday_header_all WHERE CURDATE() between `create_at` and `end_date` and FIND_IN_SET(`holiday_applied_in`, '$firm_id')");
...
//dashboard show count for no of Task of employee
        $tq = $this->db->query("SELECT COUNT(`task_assignment_id`) as task_count  FROM `customer_task_allotment_all` WHERE `alloted_to_emp_id`='$user_id'");
        if ($tq->num_rows() > 0) {
            $t_records = $tq->row();
            $task_count = $t_records->task_count;
        }

        $dq = $this->db->query("SELECT COUNT(`due_date_task_id`) as due_date_counts FROM `customer_due_date_task_transaction_all` WHERE `alloted_to`='$user_id'");
        if ($dq->num_rows() > 0) {
            $d_records = $dq->row();
            $due_date_counts = $d_records->due_date_counts;
        }

        $eq = $this->db->query("SELECT  COUNT(enquiry_id) as enquiry_count FROM `enquiry_header_all` WHERE  `allot_to`='$user_id'");
        if ($eq->num_rows() > 0) {
            $e_records = $eq->row();
            $enquiry_count = $e_records->enquiry_count;
        }

//        $lq = $this->db->query("SELECT COUNT(leave_id) as leave_count FROM leave_transaction_all WHERE `user_id`'$user_id'");
//        if ($lq->num_rows() > 0) {
//            $l_records = $lq->row();
//            $enquiry_count = $e_records->enquiry_count;
//
//            $deq = $this->db->query("SELECT COUNT(leave_id) as leave_count FROM leave_transaction_all WHERE `user_id`'$user_id'");
//            if ($lq->num_rows() > 0) {
//                $deg_records = $deq->row();
//                $total_leaves = $deg_records->enquiry_count;
//            }
//        }
        ...
        $this->load->view('employee/emp_report', $data);
    }
...
//        echo $email_id;
        ...
        $result_filled_by = $this->db->query("select work_on_services,firm_id from user_header_all where email='$email_id' and user_type='4'");
        if ($result_filled_by->num_rows() > 0) {

            foreach ($result_filled_by->result() as $row) {
                $service_name = $row->work_on_services;
                $firm_id = $row->firm_id;
            }
        } else {
...
//        echo $service_name;
        ...
        if (preg_match('/' . $special_chars . '/', $service_name)) {
            //multiple srevice  id
            $service_name_last = array();
            $multiple_service_id = explode(',', $service_name);
            for ($z = 0; $z < count($multiple_service_id); $z++) {

                $result_filled_by1 = $this->db->query("select service_name,service_id from services_header_all where service_id='$multiple_service_id[$z]' and service_type_id='' and firm_id='$firm_id'");
                if ($result_filled_by1->num_rows() > 0) {

                    foreach ($result_filled_by1->result() as $row) {
                        $service_name_last['s_name'][$z] = ['service_id' => $row->service_id, 'service_name' => $row->service_name];
                    }
                    $response['message'] = 'success';
                    $response['code'] = 200;
                    $response['status'] = true;
                } else {
                    $service_name_last['s_name'][$z] = '';
                    $response['message'] = 'No data to display';
                    $response['code'] = 204;
                    $response['status'] = false;
                }
            ...
            //single service id
            $result_filled_by1 = $this->db->query("select service_id,service_name from services_header_all where service_id='$service_name' and service_type_id='' and firm_id='$firm_id'");
            if ($result_filled_by1->num_rows() > 0) {

                foreach ($result_filled_by1->result() as $row) {
                    $service_name_last['s_name'][] = ['service_id' => $row->service_id, 'service_name' => $row->service_name];
                }
                $response['message'] = 'success';
                $response['code'] = 200;
                $response['status'] = true;
            } else {
                $service_name_last['s_name'][] = '';
                $response['message'] = 'No data to display';
                $response['code'] = 204;
                $response['status'] = false;
            }
        ...
        $response['service_names'] = $service_name_last['s_name'];
...
//                   echo substr($month1[$n], 5, 2);
    ...
    public function load_employees() {
        $firm_id = $this->input->post('firm_id');
        $user_id = array();
        $query1 = $this->db->query("select DISTINCT allot_to from enquiry_header_all where firm_id='$firm_id'");
...
//        echo $email_id;
        ...
        $result_filled_by = $this->db->query("select work_on_services,firm_id,user_id from user_header_all where email='$email_id' and user_type='4'");
        if ($result_filled_by->num_rows() > 0) {
        ...
        $year = $this->input->post('year');
        $avrage_of_completed = array();
        $result_filled_by = $this->db->query("select DISTINCT service_type_id from enquiry_header_all where firm_id='$firm_id' and allot_to='$user_id'");
        if ($result_filled_by->num_rows() > 0) {
            ...
            for ($z = 0; $z < sizeof($service_type_id); $z++) {
                ...
                $result_filled_by2 = $this->db->query("select count(enquiry_id) as total_enquiry  from enquiry_header_all where service_type_id='$service_type_id[$z]' and firm_id='$firm_id' and created_on like'$year%' and allot_to='$user_id'");
                if ($result_filled_by2->num_rows() > 0) {
                    foreach ($result_filled_by2->result() as $row) {
                        $toatal_enquiry_count[] = $row->total_enquiry;
                    }
                ...
                $result_filled_by3 = $this->db->query("select count(enquiry_id) as total_enquiry  from enquiry_header_all where service_type_id='$service_type_id[$z]' and firm_id='$firm_id' and status='6' and created_on like'$year%' and allot_to='$user_id'");
                if ($result_filled_by3->num_rows() > 0) {
                    foreach ($result_filled_by3->result() as $row) {
                        $toatal_converted_enquiry_count[] = $row->total_enquiry;
                    }
